...
def synthesize_queries(num_queries):
    trajA = abs(torch.randn(num_queries, NUM_FEATURES))
    # a = trajA[:, 0]
    # b = trajA[: , 1]
    # trajA[:, 0] = a ** 2 + b ** 2
    # trajA[:, 1] = a ** 2 + (1-b) ** 2
    # trajA[:, 2] = (1-a) ** 2 + b ** 2
    # trajA[:, 3] = (1-a) ** 2 + (1-b) ** 2
    trajB = abs(torch.randn(num_queries, NUM_FEATURES))
    # a = trajB[:, 0]
    # b = trajB[: , 1]
    # trajB[:, 0] = a ** 2 + b ** 2
    # trajB[:, 1] = a ** 2 + (1-b) ** 2
...
def respond_queries(rewardVec, trajA, trajB, give_dist = False):
    ...
    rewA = (rewardVec @ trajA.mT).squeeze()
    rewB = (rewardVec @ trajB.mT).squeeze()
...
def alignment(true_vec, mu, logvar, m=100):
    ...
    return abs(torch.mean(samples @ true_vec.T))
...
if __name__ == "__main__":
    ...
    while itr < 5000:
        ...
        mid_alignment = 0
        final_alignment = 0
        trajA, trajB = synthesize_queries(100)
        for i in range(SEQUENCE_LENGTH * BATCH_SIZE):
        ...
        loss.backward()
        optimizer.step()
        print('Iteration ' + str(itr) + ' - Loss = ' + str(loss.detach().numpy()) + ',\t Mid-Alignment = ' + str(mid_alignment.detach().numpy()) + ',\t Final-Alignment = ' + str(final_alignment.detach().numpy()))
